...
def with_step_03_instruction_tuning_system_prompt(messages: Any) -> list[Message]:
...
def normalize_explicit_preference_example(example: dict[str, Any]) -> dict[str, Any]:
    ...
    if "prompt" in example and example["prompt"] is not None:
        prompt = with_step_03_instruction_tuning_system_prompt(example["prompt"])
        return {"prompt": prompt, "chosen": chosen, "rejected": rejected}
    return {
        "chosen": with_step_03_instruction_tuning_system_prompt(chosen),
        "rejected": with_step_03_instruction_tuning_system_prompt(rejected),
    }
...
def build_reward_conversations(example: dict[str, Any]) -> tuple[list[Message], list[Message]]:
    ...
    chosen = with_step_03_instruction_tuning_system_prompt(example["chosen"])
    rejected = with_step_03_instruction_tuning_system_prompt(example["rejected"])
